...
public class WikiDumpParserCategory {
	// ==========
	// Constants
	...
	private ArrayList<PageParserCategory> arrPages;
	...
	public WikiDumpParserCategory(String fileName) {
		this.fileName = fileName;
		arrPages = new ArrayList<PageParserCategory>();
		writer = null;
		// ------------
	...
	public void parse() {
		try {
			...
			while ((line = reader.readLine()) != null) {
				...
				if (("</" + PAGE_TAG + ">").equals(line)) {
					startPage = false;
					PageParserCategory pageParser = new PageParserCategory();
	...
	private void addToBuffer(PageParserCategory pageParser) {
	...
	private void outputPages() {
		try {
			int n = arrPages.size();
			for (int i = 0; i < n; i++) {
				PageParserCategory page = arrPages.get(i);
				if (page == null)
					continue;
