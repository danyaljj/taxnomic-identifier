...
public class TextFieldSearcher extends SimpleLuceneSearcher {
	...
	public ArrayList<ILuceneResult> search(String query1, String query2, int numResults) throws Exception {

		ArrayList<ILuceneResult> results = new ArrayList<ILuceneResult>();

		query1 = formatText(query1);

		query2 = formatText(query2);

		if (query1.length() == 0 && query2.length() == 0)
			return results;

		Query query = makeQuery(query1, query2);
	...
	protected Query makeQuery(String query1, String query2) throws Exception {

		BooleanQuery bq1 = makeInitialQuery(query1);

		BooleanQuery bq2 = makeInitialQuery(query2);

		if (addNGrams) {
			bq1 = addNGrams(query1, bq1);
		}

		if (addNGrams) {
			bq2 = addNGrams(query2, bq2);
		}

		Query query = Query.mergeBooleanQueries(new Query[] { bq1,
				bq2 });
