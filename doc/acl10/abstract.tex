Determining whether two concepts in text have an ancestor relation or a sibling relation is often essential to support textual inferences such as identifying that {\em ``Jim drives a Honda"} contradicts {\em ``Jim drives a Toyota"} but both imply that {\em ``Jim drives a Japanese car"}.
Significant effort has been devoted to developing structured knowledge bases that could potentially support these tasks, but these lack sufficient coverage and, more significantly, from noise and unavoidable uncertainty, making their use in textual inference difficult.
We argue that, given two concepts, the problem of determining what taxonomic relation holds between them should be addressed by fusing information from multiple knowledge sources both structured and unstructured.
